/*
   Arduino RC Receiver - Car Example           WORKING 3/28/21 Need to add button commands now LRC
                                               Joystick Button 1 is now working, see last code lines LRC
...
{ 
  ...
  wp1Speed = data.pot1;
  // wp1Speed = map(wp1Speed, 255, 0, 0, 255); use this to change dirrection of slider
   }
    if (wp1Speed < 70) {
      wp1Speed = 0;
    }
  
  analogWrite(wp1, wp1Speed); // Send PWM signal to weapon 1  (weapon one)
...
void resetData() {
  ...
  data.j1PotX = 127;
  data.j1PotY = 127;  
  data.j2PotX = 127;   // steering axis
  data.j2PotY = 127;  // trottle axis
  data.j1Button = 1;
  data.j2Button = 1;
  data.pot1 = 1;
  data.pot2 = 1;
